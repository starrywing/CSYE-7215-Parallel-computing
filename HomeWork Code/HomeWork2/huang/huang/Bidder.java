        ...
        for (int i = 0; (i < cycles && cash > 0) || activeBids.size() > 0; ++i)
        {
            ...
            while (items.size() > 0)
            {
                ...
                if (price < this.cash - sumActiveBids)
                {
                	...
                	//System.out.println(this.name()+" try to submit bid item_"+item.listingID()+" at price $"+(price+1));
                    boolean success = server.submitBid(this.name(), item.listingID(), price + 1);
                    if (success)
                    {   //System.out.println(this.name()+" submit bid item_"+item.listingID()+" succeed"+(price + 1));
            ...
            {
            	//System.out.println(this.name()+" bid item_"+bid.listingID());
                switch (server.checkBidStatus(this.name(), bid.listingID()))
                {
                    case 1:
                        // Success
                    	//System.out.println(this.name()+" Success item_"+bid.listingID());
                    ...
                    case 3:
                        // Failed
                    	//System.out.println(this.name()+"Failed item_"+bid.listingID());
